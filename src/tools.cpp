...
{
    // the default folder of the lmdb blockchain database
    string default_lmdb_dir = gntleg::get_default_lmdb_folder(nettype);
    ...
    blockchain_path = gntleg::remove_trailing_path_separator(blockchain_path);
...
{
    pair<uint64_t, uint64_t> sum_gntl {0,0};
    ...
    {
        sum_gntl.first += vout["amount"].get<uint64_t>();
        ++sum_gntl.second;
    }
...
{
    pair<uint64_t, uint64_t> sum_gntl {0ULL,0ULL};
        ...
        vector<txin_to_key> &input_key_imgs)
{
    ...
    uint64_t gntl_outputs       {0};
    uint64_t gntl_inputs        {0};
    uint64_t mixin_no          {0};
    ...
    for (size_t i = 0; i < input_no; ++i)
    {
        ...
        gntl_inputs += tx_in_to_key.amount;
    ...
    return {gntl_outputs, gntl_inputs, mixin_no, num_nonrct_inputs};
};
...
summary_of_in_out_rct(const json &_json)
{
    ...
    for (const json &vout: _json["vout"])
    {
        gntl_outputs += vout["amount"].get<uint64_t>();
    }
    ...
    return {gntl_outputs, gntl_inputs, no_outputs, no_inputs, mixin_no, num_nonrct_inputs};
};
...
sum_money_in_inputs(const string &json_str)
{
    ...
    for (json &vin: j["vin"])
    {
        sum_gntl.first += vin["key"]["amount"].get<uint64_t>();
        ++sum_gntl.second;
    }
...
{
    array<uint64_t, 2> sum_gntl;

    sum_gntl[0] = sum_money_in_inputs(tx);
    sum_gntl[1] = sum_money_in_outputs(tx);
