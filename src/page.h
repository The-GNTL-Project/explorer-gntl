...
#define GNTLEXPLORER_RPC_VERSION_MAJOR 1
#define GNTLEXPLORER_RPC_VERSION_MINOR 0
#define MAKE_GNTLEXPLORER_RPC_VERSION(major,minor) (((major)<<16)|(minor))
#define GNTLEXPLORER_RPC_VERSION \
    MAKE_GNTLEXPLORER_RPC_VERSION(GNTLEXPLORER_RPC_VERSION_MAJOR, GNTLEXPLORER_RPC_VERSION_MINOR)
    ...
    while (i >= start_height)
    {
        ...
        if (enable_block_cache && block_tx_cache.Contains(i))
        {
            ...
            for (const pair<crypto::hash, mstch::node> &txd_pair: txd_pairs)
            {
                ...
                if (i + config::tx_settings::GNTL_TX_CONFIRMATIONS_REQUIRED > height)
                {
...
search(string search_text)
{
    ...
    // check if GNTL address is given based on its length
    ...
    // check if integrated GNTL address is given based on its length
             ...
             bool tx_prove = false)
{
    ...
    {
        j_response["status"]  = "error";
        j_response["message"] = "GNTL address not provided";
        return j_response;
    }
...
json_networkinfo()
{
    ...
    if (!get_gntl_network_info(j_info))
    {
        j_response["status"]  = "error";
        j_response["message"] = "Cant get GNTL network info";
        return j_response;
    }
...
json_version()
{
    ...
    j_data = json {
            ...
            {"gntl_version_full"  , string {GNTL_VERSION_FULL}},
            {"api"                 , GNTLEXPLORER_RPC_VERSION},
...
bool
get_gntl_network_info(json &j_info)
{
...
get_footer()
{
    ...
    static const mstch::map footer_context {
            ...
            {"git_branch_name"     , string {GIT_BRANCH_NAME}},
            {"gntl_version_full"  , string {GNTL_VERSION_FULL}},
            {"api"                 , std::to_string(GNTLEXPLORER_RPC_VERSION_MAJOR)
                                     + "."
